...
export function getProjectOfSession(
  session: Session,
  user: User
): Project | undefined {
...
export function getTaskOfSession(
  session: Session,
  user: User
): Task | undefined {
...
export function calcFlowComparison(
...
export function calcDurationComparison(user: User, session: Session) {
  ...
  const avgDurationOfOtherSessions =
    calculateAverageDurationOfOtherSessions(user);
