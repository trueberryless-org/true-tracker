...
const calculateChartData = (user: User, dateRange: DateRange | undefined) => {
  ...
  const chartData = flows.map((flow) => {
    const flowSessions = filteredSessions.filter(
      (session) => session.flow === flow.value
    );
    const priorityCounts = priorities.reduce(
      (counts, priority) => {
        counts[priority.value] = flowSessions.filter(
          (session) => session.priority === priority.value
        ).length;
        return counts;
      },
...
function getHelperText(
  chartData: { flow: string }[] | { [x: string]: any; flow: any }[]
): React.ReactNode {
  ...
  const flowData = chartData.map(({ flow, ...priorities }) => {
    const totalByFlow = Object.values(priorities).reduce(
...
export function ChartSessionFlowRadar({
  user,
  dateRange,
  className,
}: ChartSessionFlowRadarProps) {
