...
function getAllIntervals(
  ...
  switch (interval) {
    case "month":
      intervals = eachMonthOfInterval({ start, end }).map((date) =>
        format(date, "yyyy-MM")
      );
      break;
    case "week":
      ...
      break;
    case "day":
      intervals = eachDayOfInterval({ start, end }).map((date) =>
...
function aggregateFlowData(
  ...
  sessions.forEach((session) => {
    ...
    while (currentDate <= endDate) {
      const intervalKey = getAllIntervals(
        currentDate,
        currentDate,
        interval
      )[0];
        ...
        <ChartContainer config={chartConfig}>
          <BarChart data={flowData}>
            ...
            <Bar
              dataKey="disrupted"
              stackId="a"
              fill={chartConfig.disrupted.color}
            />
          </BarChart>
        </ChartContainer>
      </CardContent>
