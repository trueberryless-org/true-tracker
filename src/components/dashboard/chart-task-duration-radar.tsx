...
function generateChartData(user: User): { chartData: any; chartConfig: any } {
  ...
  const task2 = getTaskWithMostSessionDurationInInterval(
    user,
    dateRange,
    task1 ? [task1.id] : []
  );
...
export function ChartTaskDurationRadar({
  user,
  className,
}: ChartTaskDurationRadarProps) {
  ...
  return (
    <Card className={className}>
      <CardHeader className="items-center pb-4">
        ...
        <CardDescription>
          Showing total time spent on the most worked on tasks.
        </CardDescription>
      </CardHeader>
