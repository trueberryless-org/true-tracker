...
const calculateChartData = (user: User, dateRange: DateRange | undefined) => {
    ...
    if (dateRange) {
        projectTaskCounts = projects.map((project) => ({
            ...
            taskCount: project.tasks.filter((task) =>
                task.sessions.some((session) => isSessionInDateRange(session, dateRange)),
            ).length,
        }));
    }
    ...
    const otherTaskCount = otherProjects.reduce((acc, project) => acc + project.taskCount, 0);
...
export function ChartTaskAmount({ user, dateRange, className }: ChartTaskAmountProps) {
        ...
        <Card className={`${className} flex flex-col`}>
            ...
            <CardFooter className="flex-col gap-2 text-sm">
                ...
                <div className="leading-none text-muted-foreground">
                    Showing top {chartData.filter((data) => data.tasks !== 0).length} project
