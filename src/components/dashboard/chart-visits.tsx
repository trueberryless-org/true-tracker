...
function generateChartDataYear(user: User): [{ column: string; visits: number }[], number, number] {
    ...
    const totalVisits = Object.values(yearlyVisits).reduce((sum, count) => sum + count, 0);
    ...
    const averageVisitsPerYear = numberOfYearsWithVisits > 0 ? totalVisits / numberOfYearsWithVisits : 0;

    return [chartData, currentYearVisits, averageVisitsPerYear];
}

function generateChartDataMonth(user: User): [{ column: string; visits: number }[], number, number] {
    const monthNames = [
        "January",
        "February",
        "March",
...
function generateChartDataWeek(user: User): [{ column: string; visits: number }[], number, number] {
...
function calculateVisitPercentage(currentVisits: number, averageVisits: number): string {
...
export function ChartVisits({ user, className }: ChartVisitsProps) {
    const [chartDataYear, currentYearVisits, averageVisitsPerYear] = generateChartDataYear(user);
    const [chartDataMonth, currentMonthVisits, averageVisitsPerMonth] = generateChartDataMonth(user);
        ...
        <Tabs defaultValue="week" className={`${className} mt-0`}>
                ...
                <Card className="h-full flex flex-col justify-between">
                    ...
                    <CardFooter className="flex-col items-start gap-2 text-sm">
                        <div className="flex gap-2 font-medium leading-none">
                            {calculateVisitPercentage(currentWeekVisits, averageVisitsPerWeek)} of average on{" "}
                            {
                                ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][
                                    new Date().getDay()
                                ]
                            }{" "}
                            {currentWeekVisits / averageVisitsPerWeek >= 1 && <TrendingUp className="h-4 w-4" />}
                            {currentWeekVisits / averageVisitsPerWeek < 1 && <TrendingDown className="h-4 w-4" />}
                        </div>
            ...
            <TabsContent value="month" className="mt-0 h-full">
                <Card className="h-full flex flex-col justify-between">
                    ...
                    <CardFooter className="flex-col items-start gap-2 text-sm">
                        <div className="flex gap-2 font-medium leading-none">
                            {calculateVisitPercentage(currentMonthVisits, averageVisitsPerMonth)} of average in{" "}
                            {
                                [
                                    "January",
                                    "February",
                            ...
                            {currentMonthVisits / averageVisitsPerMonth >= 1 && <TrendingUp className="h-4 w-4" />}
                            {currentMonthVisits / averageVisitsPerMonth < 1 && <TrendingDown className="h-4 w-4" />}
                        </div>
                        <div className="leading-none text-muted-foreground">
            ...
            <TabsContent value="year" className="mt-0 h-full">
                <Card className="h-full flex flex-col justify-between">
                    ...
                    <CardFooter className="flex-col items-start gap-2 text-sm">
                        <div className="flex gap-2 font-medium leading-none">
                            {calculateVisitPercentage(currentYearVisits, averageVisitsPerYear)} of average in{" "}
                            {new Date().getFullYear()}{" "}
                            {currentYearVisits / averageVisitsPerYear >= 1 && <TrendingUp className="h-4 w-4" />}
                            {currentYearVisits / averageVisitsPerYear < 1 && <TrendingDown className="h-4 w-4" />}
                        </div>
                        <div className="leading-none text-muted-foreground">
