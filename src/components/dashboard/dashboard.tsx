...
import {
  getProjectWithMostSessionDuration,
  getProjectWithMostSessionDurationInInterval,
...
import {
  getMostRecentSessionDateInIntervalOfUser,
  getMostRecentSessionDateOfUser,
...
export default function Dashboard() {
  ...
  const shouldShowExportNotification = (user: User) => {
    ...
    return (
      (user.settings.exportReminder === "daily" &&
        lastExportedTime <= oneDayAgo) ||
      (user.settings.exportReminder === "weekly" &&
        lastExportedTime <= oneWeekAgo) ||
      (user.settings.exportReminder === "monthly" &&
        lastExportedTime <= oneMonthAgo)
    );
  };
  ...
  function getWorkingTimeOfUser(
    ...
    return user.projects
      ...
      .filter((session) => {
        return (
          (dateRange ? isSessionInDateRange(session, dateRange) : true) &&
          flows.includes(session.flow)
        );
      })
  ...
  function getProjectDurationInfoText(user: User): string {
    ...
    if (maxDuration > 0 && secondMaxDuration > 0) {
      const percentageIncrease =
        ((maxDuration - secondMaxDuration) / secondMaxDuration) * 100;
  ...
  function getProjectDurationInfoTextInInterval(
    user: User,
    dateRange: DateRange
  ): string {
    let maxDuration = 0;
  ...
  function getTaskDurationInfoTextInInterval(
    user: User,
    dateRange: DateRange
  ): string {
    let maxDuration = 0;
  ...
  function getRecentTasksInfoText(
    user: User,
    dateRange: DateRange | undefined
  ): string {
  ...
  function getRecentSessionsInfoText(
    user: User,
    dateRange: DateRange | undefined
  ): string {
    if (dateRange == undefined) {
              ...
              <TabsTrigger value="notifications">
                <div className="flex items-center space-x-2">
                  ...
                  {hasNotifications(user) && (
                    <Bell
                      className={`h-4 w-4 ${!notificationsRead ? "text-primary" : ""}`}
                    />
                  )}
                </div>
              </TabsTrigger>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                  ...
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {dateRange
                          ? getProjectDurationInfoTextInInterval(
                              user,
                              dateRange
                            )
                  ...
                  <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                      <CardTitle className="text-sm font-medium text-primary">
                        Last time working{" "}
                        {dateRange && dateRange.to ? "in given interval" : ""}
                      </CardTitle>
                ...
                <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-6 xl:grid-cols-7">
                  ...
                  <Card className="order-2 md:col-span-2 md:max-lg:order-3 lg:col-span-4 xl:col-span-3 overflow-hidden">
                    <CardHeader>
                      ...
                      <CardDescription>
                        {getRecentTasksInfoText(user, dateRange)}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
              ...
              {!hasNotifications(user) && (
                <Card className="col-span-4 lg:col-span-3">
                  <CardHeader>
                    ...
                    <CardDescription>
                      You have no notifications. If you had notifications, there
                      would be a <Bell className="h-4 w-4 inline-block" /> icon
                      right next to the &quot;Notifications&quot; tab.
                    </CardDescription>
                  </CardHeader>
                </Card>
              )}
